...
class LoadTextAnnotations(BaseTransform):
    ...
    def get_extra_classes(self, true_classes: tuple, all_classes: tuple) -> tuple:
        ...
        max_extra_labels = 4
        probabilities = {
            i: 1 / (max_extra_labels + 1) for i in range(max_extra_labels + 1)
        }
        ...
        filtered_classes = [
            ...
            if c not in true_classes and c not in excluded_classes
        ]
    ...
    def transform(self, results: dict) -> dict:
        ...
        if "phrases" in results:
            tokens_positive = [
                phrase["tokens_positive"] for phrase in results["phrases"].values()
            ]
